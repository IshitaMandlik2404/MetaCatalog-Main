...
export default function Home() {
  ...
  // Role persisted by Login.js after calling /api/auth/record-login
  // Possible values: 'viewer' | 'editor' | 'admin' | 'system_admin'
  const role = (localStorage.getItem('app_role') || 'viewer').toLowerCase();

  const canSeeConfig = ['editor', 'admin', 'system_admin'].includes(role);
  const canSeeInput  = ['editor', 'admin', 'system_admin'].includes(role);
  const canSeeSearch = true; // everyone can see search
